...
def del_color(x):
    if 'itemStyle' in x.keys():
        if 'color' in x['itemStyle'].keys():
            del x['itemStyle']['color']
    if 'children' in x.keys():
        x['children'] = [del_color(y) for y in x['children']]
    return x

data_ = [del_color(x) for x in data]
...
utils_html.open_html("drink_flavors.html")
